...
### 2.1 接口位置
- **文件**：`SeverTest/Controllers/StrategyController.cs`
...
### 7.2 后端文件
- `SeverTest/Controllers/StrategyController.cs` - 删除接口实现
- `SeverTest/Services/RealTimeStrategyEngine.cs` - 运行时策略引擎
- `SeverTest/Services/StrategyRuntimeBootstrapHostedService.cs` - 策略启动服务

### 7.3 数据库相关
- `SeverTest/Infrastructure/Repositories/StrategyPositionRepository.cs` - 仓位仓储
- `SeverTest/Infrastructure/Repositories/StrategyRunCheckLogRepository.cs` - 检查日志仓储
