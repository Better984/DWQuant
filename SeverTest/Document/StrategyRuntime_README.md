...
- 新增条件 Key 本地缓存：同一策略同一条件在运行期复用已生成的 `ConditionKey`，减少热路径字符串拼接与 MD5 计算。
- 策略上下线时清理条件 Key 缓存：避免频繁上下线导致缓存条目持续增长。
- 指标请求在策略载入时预注册：自动预计算链路可直接命中句柄，减少首次触发时的临时构建开销。
- 指标同步刷新增加“已预计算水位”判断：若自动预计算已覆盖当前任务，则跳过同步刷新，降低重复计算。
- 运行画像 `extra_json` 写入受 `StrategyDiagnostics:PersistRunProfileExtraJson` 开关控制，关闭后不再序列化该字段。
