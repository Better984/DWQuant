...
- 历史数据查询是内存操作，速度很快
- `GetHistoryKlines` 采用尾部窗口构建，避免整段列表复制与额外 LINQ 分配
- 收线任务改为“锁内收集、锁外分发”，降低 `SymbolCache` 锁持有时间
- 分片策略消费队列建议使用可观测丢弃策略（`DropWrite`），避免静默覆盖旧任务
...
### v1.1.1 (2026-02-27)
- 优化历史窗口查询逻辑：按需截取尾部数据，减少内存分配
- 调整任务分发路径：将任务入队从缓存锁内移至锁外，降低竞争
- 补充实盘高并发场景下的分片队列策略建议
