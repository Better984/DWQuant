...
**位置**：`SeverTest/Services/HistoricalMarketDataSyncHostedService.cs`  
**作用**：启动时执行历史行情同步与预热流程。
...
**位置**：`SeverTest/Services/HistoricalMarketDataSyncService.cs`  
**作用**：负责增量同步历史行情到数据库。
...
### 3. HistoricalMarketDataCache
**位置**：`SeverTest/Services/HistoricalMarketDataCache.cs`  
**作用**：历史数据缓存与按需回表查询。
...
**位置**：`SeverTest/Controllers/MarketDataController.cs`  
...
位置：`SeverTest/Monitoring/StartupMonitorForm.cs`
