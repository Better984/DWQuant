...
**位置**：`SeverTest/Modules/MarketData/Infrastructure/HistoricalMarketDataRepository.cs`  
**作用**：集中处理历史行情表的查询和写入（表存在检查、最后时间查询与批量写入），保证 SQL 仅出现在仓储层。
...
## 变更说明（2026-02）
- 同步范围：保持仅 `Binance + BTC_USDT`，按全部周期执行增量同步。
- 同步间隔策略：最小同步间隔按周期收敛，避免 1m K 线因为 `SyncMinGapMinutes` 配置过大而持续落后。
- 历史接口补尾：`MarketDataController.GetHistoryKlines` 会在历史缓存/数据库结果上叠加 `MarketDataEngine` 实时尾部并按时间戳去重。
- 实际效果：当数据库尚未完成本轮落库时，`/api/marketdata/history` 仍可返回连续的最新尾部K线，前端图表缺口显著减少。
- 默认配置建议：`HistoricalData:SyncIntervalMinutes` 由 `60` 调整为 `1`；若使用 `server_config` 覆盖，请同步修改并重启服务。
