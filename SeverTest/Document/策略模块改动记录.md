...
- 回测结果“交易明细”支持仓位点击联动：
  ...
  - 仓位聚焦体验优化（2026-02-28）：
    - 自动按持仓时长计算左右冗余（每侧默认约为持仓K线数的 25%，左右保持一致），点击后直接定位到可见区域，减少手动拖拽。
    - 若“持仓K线数 + 冗余”超过当前画布最大可见K线数，自动上调到更大周期（如 1m -> 5m -> 15m ...）直至可完整显示。
    - 自动定位实现改为按 dataIndex 居中，避免 `scrollToTimestamp` 在极端数据下触发 `binarySearchNearest` 异常。
    - 图表边界加载改为双向增量：左滑触发 `forward`、右滑触发 `backward`，两侧均通过 `setLoadDataCallback` 追加数据，不再整包 `applyNewData` 重置，降低视图跳动。
- 回测联动链路新增前端调试日志（开发环境）：
...
- `MarketChart` 标准化重构（2026-02-14）：
  ...
  - 本地源码跨设备同步兜底（2026-02-15）：
    - 新增 `Client/scripts/ensure-local-klinecharts.mjs`，在 `dist` 缺失时自动执行依赖安装与构建。
    - `Client/package.json` 的 `dev/build` 脚本接入 `ensure:klinecharts-local`，避免新设备直接启动时报 `Failed to resolve import "klinecharts"`。
    - 文档补充：新增 `SeverTest/Document/KLineCharts本地源码同步指南.md`。
  ...
  - `MarketChart` 风险区绘图工具扩展（2026-02-16）：
    - 左侧绘图栏新增第 6 组 `risk`，包含 `riskRewardLong`（多头）与 `riskRewardShort`（空头）两类止盈止损区间工具。
    - 交互调整为“直接绘图模式”：与其它绘图工具一致，点击后直接在图上选区间，不再弹出参数面板。
    - 选区间后自动生成默认 TP/SL（可拖拽调整，默认止盈/止损均为开仓价 1% 偏移），并按区间 K 线实时推导“真实交易区间”：
      - 开始点取区间内第一根触及开仓价的 K 线；
      - 结束点优先取触及 TP/SL 的 K 线，否则取区间最后一根 K 线收盘价。
    - 风险区文字（开仓/止盈/止损/真实区间）改为按交互显示：仅在鼠标悬停或选中风险区时显示，非悬停且非选中时隐藏。
    - 风险区工具栏图标改为独立 SVG 资源（`24_risk_reward_long.svg`、`25_risk_reward_short.svg`），线条风格与位置对齐现有绘图工具图标体系。
    - 风险区覆盖层支持继续拖拽调整，颜色标准对齐回测仓位聚焦：止盈计划区、止损计划区、已实现盈利区、已实现亏损区。
    - 开仓到真实结束点的虚线样式改为更细、更淡，避免遮挡主图 K 线观察。
    - `MarketChart` 新增可选 API 回调 `onRiskZoneApiReady`，暴露 `addRiskZone / replaceRiskZone / removeRiskZone / clearRiskZones / listRiskZones`，支持外部按业务数据驱动绘制与管理风险区。
    - 同步更新文档：`Client/src/components/MarketChart.DrawingBar.布局说明.md`。
  - `MarketChart` 仓位焦点右滑稳定性修复（2026-02-28）：
    - 左右分页改为“已加载时间边界”推进：左滑按当前最小时间戳继续向前拉取，右滑按当前最大时间戳继续向后拉取，避免可视锚点抖动导致重复加载或跳图。
    - 焦点模式下暂停 `subscribeMarket` 实时推送，防止浏览历史仓位时被最新实时 K 线插入造成视图突跳。
    - 仓位焦点周期改为“新仓位仅自动对齐一次”：首次点击新仓位时按建议周期/可视上限自动选周期；后续用户手动切换周期（如 1m -> 5m）不再被强制拉回原周期。
    - 修复周期回切根因：焦点模式下不再持续执行外部 `interval` 同步，避免父组件固定传入的回测周期把用户手动切换立即覆盖。
    - 点击仓位首屏加载增加更大的右侧预取，优先一次性拉到“焦点区间 + 右侧缓冲”，降低首屏进入后立刻触发右侧补拉的概率。
    - 焦点首屏新增短暂 `Backward` 自动补拉抑制窗口，仅屏蔽初始化阶段触发，抑制“第一次点击仓位即回跳到最新”的现象。
    - 本地 `klinecharts` 源码（`ChartStore.addData` 的 `LoadDataType.Backward` 分支）增加“右侧追加保持锚点”补偿：追加 N 根时同步修正右侧偏移，避免每次补拉后视图被推向最新。
    - 分页异常时不再将 `more` 置为 `false`，保留继续加载能力，支持用户在网络抖动后继续右滑/左滑重试。
    - 打开“量化回测专用界面”时新增默认聚焦规则：当未手动点选仓位时，自动定位到最新开仓仓位（按 `entryTime` 最近优先）。
    - 图表新增策略指标预设：从策略配置逻辑中提取指标引用（`refType=indicator` 等引用结构），进入回测专用界面后自动只展示策略使用的指标（如 `EMA + MA` 或 `EMA + MACD + MA`）。
    - 指标预设参数链路补齐：除指标代码外，新增透传并应用 `params` 与 `input`，按指标 schema 落地到 `calcParams/extendData.taInputMap`，避免界面仍显示默认参数。
    - 未平仓仓位支持焦点绘制：当 `exitTime` 缺失或无效时，联动区间终点改为“最新时间 + 10 根K线”，避免未平仓点击后无绘制问题，并直观体现持仓仍在进行中。
    - 回测后端收尾口径调整：样本结束时不再按最后价格强制平仓，改为输出 `isOpen=true` 的未平仓快照；前端据此显示“未平仓”原因与时间范围，并在点击时按“最新时间 + 10 根K线”进行联动绘制。
