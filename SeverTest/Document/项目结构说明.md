...
- 指标框架：`Indicators/IndicatorQueryService` 提供 L1/L2/L3 缓存链路与单飞刷新，`Indicators/IndicatorRefreshHostedService` 定期补齐快照并维护历史点位，`Indicators/CoinGlassStreamBridgeHostedService` 可选接入聚合商 WS 拉流并通过 HTTP 输出缓存；开发阶段允许临时使用第三方非官方聚合商（盗版源）联调，正式上线前需切换官方源；默认内置 `coinglass.fear_greed`、`coinglass.etf_flow`、`coinglass.liquidation_heatmap_model1`；`indicator_definitions.config_json` 统一描述来源类型与字段目录（fields），策略端通过 `StrategyEngine/PublicIndicatorValueProvider` 支持 `RefType=PublicIndicator` 直接引用公共信号字段。
...
- `Client/src/components/indicator/IndicatorModule.tsx`：指标中心采用组件化指标卡片并接入 ECharts；其中“贪婪恐慌指数”“比特币现货 ETF 净流入”“交易对爆仓热力图（模型1）”已接入后端 `indicator.latest.get` 实时接口，其余卡片保留静态示例展示。爆仓热力图支持主图 K 线叠加，并在右侧输出动态 100 桶价格分布、上涨/下跌最大痛点及当前价上下累计清算（订单簿式）。
