...
- Discover 资讯与日历：`Discover/Feed/Application/DiscoverFeedRefreshHostedService` 按配置间隔定时拉取新闻与快讯；`Discover/Calendar/Application/DiscoverCalendarRefreshHostedService` 按配置间隔定时拉取三类日历，去重键分别为 `title+release_time+source`（资讯）与 `calendar_name+publish_timestamp+country_code(+country_name)`（日历），数据库全量保留且内存缓存仅保留最近 N 条（默认 1000），对外提供 `discover.article.pull`、`discover.newsflash.pull` 与三条 `discover.calendar.*.pull` 协议。
