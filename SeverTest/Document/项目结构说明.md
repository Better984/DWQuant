...
- 服务器角色启动：`Shared/Application/Services/ServerRoleBootstrap.cs` 在启动时支持三步交互（选择角色→是否更新分布式核心节点地址→确认启动），并持久化到 `Config/server-role.local.json`；角色包含 `core`、`backtest-worker`、`full`、`live-trading-worker`。
- WebSocket 跨节点挤线：`Program.cs` 在 `KickOld` 策略下会先广播分布式踢线，再进行本地挤出，提升多节点连接一致性；连接槽位成员采用 `nodeId|connectionId`，并支持“按当前节点定向清理陈旧槽位”兜底，降低重启后 403 误判与跨节点误删风险。
...
- 策略多实例租约：`StrategyRuntime/StrategyOwnershipService` + `StrategyRuntimeLeaseHostedService` 通过 Redis 租约保证策略单实例执行，并支持接管与同步；续租与同步已拆分独立循环，避免同步阻塞导致续租超时。
...
- 分布式实盘节点：核心节点通过 `StrategyRuntime/LiveTradingWorkerGateway` 接收实盘节点连接，并由 `LiveTradingWorkerDispatchService` 分发 `upsert/remove` 指令；实盘节点通过 `LiveTradingWorkerClientHostedService` 接收并驱动本地 `RealTimeStrategyEngine` 增删策略。
...
- 指标框架：`Indicators/IndicatorQueryService` 提供 L1/L2/L3 缓存链路与单飞刷新，`Indicators/IndicatorRefreshHostedService` 定期补齐快照并维护历史点位，默认内置 `coinglass.fear_greed`。
- 策略链路诊断：`StrategyDiagnostics` 配置统一控制策略运行画像（分段耗时/extra_json）、行情任务分发耗时、动作入队耗时与交易动作消费耗时，便于按模块定位瓶颈。
- 任务全链路追踪：`Shared/Application/Diagnostics/StrategyTaskTraceLog*` + `strategy_task_trace_log` 表统一记录 `trace_id` 路径（行情分发→策略认领/执行→动作入队→交易消费），并落地处理实例 `actor_instance` 用于分布式定位。
- 任务主记录：`StrategyEngine/StrategyEngineRunLog*` + `strategy_engine_run_log` 表按“单任务单行”沉淀主画像（`trace_id/run_status/分段耗时/策略集合/open_task_trace_ids/open_order_ids`），管理端市场任务报告优先基于该主表查询，降低对事件大表聚合依赖。
...
- `Client/src/components/indicator/IndicatorModule.tsx`：指标中心采用组件化指标卡片并接入 ECharts；其中“贪婪恐慌指数”已接入后端 `indicator.latest.get` 实时接口，其余卡片保留静态示例展示。
