...
- `AiAssistant/`：AI 助手对话与策略 JSON 生成（DeepSeek 接入、知识库注入、结果归一化、会话持久化）。
...
- AI 助手：`AiAssistant` 模块提供会话列表/新建/历史/聊天接口（`/api/ai-assistant/conversations/*`、`/api/ai-assistant/chat`），会话与消息落库后可在换设备/重新登录后继续历史对话。
