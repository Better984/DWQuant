...
2) **启动时不会自动加载 DB 中 running 策略**
- `StrategyRuntimeHostedService` 只启动引擎 Worker，`LoadTestStrategy()` 已注释。
- 运行策略依赖手动调用 `PATCH /api/strategy/instances/{id}/state`。
- 涉及文件：`SeverTest/Services/StrategyRuntimeHostedService.cs`
