...
- 星球模块参数示例：
  - `Planet:PostOwnerCanDeleteOthersComments`：贴主是否可删除自己帖子下其他用户评论（实时生效）。
- 回测并行相关参数示例：
...
- 分布式实盘与节点通信参数示例：
  - `LiveTradingWorker:HeartbeatSeconds`：实盘节点心跳间隔（需重启）。
  - `LiveTradingWorker:ReconnectDelaySeconds`：实盘节点断线重连间隔（需重启）。
  - `LiveTradingWorker:CoreWsUrls`（`|` 分隔多地址）、`LiveTradingWorker:CoreWsUrl`（单地址兼容）与 `LiveTradingWorker:AccessKey` 建议保留在 `appsettings*.json`（路由/密钥敏感项，不建议入库）。
- 行情订阅存储参数示例：
  - `MarketStreaming:UseRedisSubscriptionStore`：是否使用 Redis 存储行情订阅关系（需重启）。
- WebSocket 节点参数示例：
  - `WebSocket:NodeId`：节点标识后缀（服务会自动拼接机器名前缀，需重启）；多实例部署建议显式配置，保证同机多实例唯一且跨重启稳定。
