...
目录位置：
- `SeverTest/Modules/StrategyEngine/Application/RunChecks/`

## 当前检查项
1) API 校验  
   - 文件：`SeverTest/Modules/StrategyEngine/Application/RunChecks/Checks/ApiKeyRunCheck.cs`
   - 逻辑：验证策略绑定的 API Key 是否存在。

2) 仓位模式校验  
   - 文件：`SeverTest/Modules/StrategyEngine/Application/RunChecks/Checks/PositionModeRunCheck.cs`
   - 逻辑：调用交易所接口获取账户仓位模式，与策略配置中的 `positionMode` 比对。
   - 期望值：`Cross`（全仓）/ `Isolated`（逐仓）

3) 余额校验  
   - 文件：`SeverTest/Modules/StrategyEngine/Application/RunChecks/Checks/BalanceRunCheck.cs`
