...
- **约定**：本文 API 输出基于 `ApiResponse<T>`，时间参数使用 `yyyy-MM-dd HH:mm:ss`（默认本地时区）或 ISO8601（推荐 UTC）。
...
| position_mode | VARCHAR(16) | 否 | LongShort/Hedge |
| leverage | INT | 否 | 杠杆 |
...
CREATE TABLE strategy_position (
  ...
  position_mode VARCHAR(16) NOT NULL COMMENT '仓位模式: LongShort/Hedge',
...
```
- **时间格式**：
  - 推荐：ISO8601 UTC（如 `2026-01-27T08:00:00Z`）
  - 兼容：`yyyy-MM-dd HH:mm:ss`（默认本地时区）
...
```json
{
  ...
  "lastPrice": 43750.0,
  "lastCheckAt": "2026-01-27 16:05:00",
  "closeReason": null,
  "openOrderId": 20001,
  "closeOrderId": null,
  "openedAt": "2026-01-27 16:01:00",
