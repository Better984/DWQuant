...
- alias_name
- description
- state (completed/running/paused/paused_open_position/paused_open_fail/archived；历史数据可能有 draft/ready；paused_open_fail 为连续开仓失败自动暂停)
...
```json
{
  ...
  "description": "备注",
  "aliasName": "用户侧名称",
  "configJson": { "trade": {}, "logic": {}, "runtime": {} }
}
```
...
校验：name<=64，description<=255，aliasName<=64；运行时间配置需满足自定义按小时精度、模板可分钟。
...
请求：
```json
{ "usId": 5001, "configJson": { ..., "runtime": {} }, "changelog": "..." }
```
