...
namespace ServerTest.Services
{
    ...
    public static class OhlcvCacheTest
    {
            ...
            int? maxLimitPerRequest = null)
        {
            ...
            // 每次请求的最大条数
            var limitPerRequest = maxLimitPerRequest ?? new MarketDataQueryOptions().MaxLimitPerRequest;

            while (result.Count < targetBars)
            {
                var remaining = targetBars - result.Count;
                var limit = Math.Min(remaining, limitPerRequest);
