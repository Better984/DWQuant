...
{
    /// <summary>
    /// CoinGlass Hyperliquid 持仓排行采集器。
    /// </summary>
    public sealed class CoinGlassHyperliquidPositionCollector : IIndicatorCollector
    {
        private readonly CoinGlassClient _coinGlassClient;
        private readonly CoinGlassOptions _coinGlassOptions;
        private readonly ILogger<CoinGlassHyperliquidPositionCollector> _logger;

        public CoinGlassHyperliquidPositionCollector(
            CoinGlassClient coinGlassClient,
            IOptions<CoinGlassOptions> coinGlassOptions,
            ILogger<CoinGlassHyperliquidPositionCollector> logger)
        {
        ...
        public string CollectorName => "coinglass.hyperliquid_position.collector";
        ...
        public bool CanHandle(IndicatorDefinition definition)
        {
            ...
            return string.Equals(definition.Provider, "coinglass", StringComparison.OrdinalIgnoreCase)
                && string.Equals(definition.Code, "coinglass.hyperliquid_position", StringComparison.OrdinalIgnoreCase);
        }
        ...
        {
            var symbol = ResolveScopeValue(scopeKey, _coinGlassOptions.HyperliquidPositionDefaultSymbol, "symbol")
                .Trim()
                .ToUpperInvariant();

            _logger.LogInformation(
                "[coinglass][Hyperliquid][持仓排行] 开始拉取持仓排行: symbol={Symbol}, scopeKey={ScopeKey}",
                symbol,
                scopeKey);

            using var response = await _coinGlassClient
                .GetHyperliquidPositionAsync(symbol, ct)
            ...
            {
                throw new InvalidOperationException("CoinGlass Hyperliquid 持仓排行接口返回为空，无法生成指标数据");
            }

            var displayItems = orderedItems
                .Take(Math.Max(1, _coinGlassOptions.HyperliquidPositionTopCount))
                .ToList();
        ...
        private static ParsedHyperliquidPositionResponse ParseResponse(JsonElement root, string symbol)
        {
            CoinGlassCollectorJsonHelper.ValidateResponseCode(root, "Hyperliquid 持仓排行");

            if (root.ValueKind != JsonValueKind.Object ||
                !root.TryGetProperty("data", out var dataRoot) ||
                dataRoot.ValueKind != JsonValueKind.Object)
            {
                throw new InvalidOperationException("CoinGlass Hyperliquid 持仓排行接口返回结构异常，未找到 data 对象");
            }

            var totalPages = (int)(CoinGlassCollectorJsonHelper.TryReadLong(dataRoot, "total_pages", "totalPages") ?? 0L);
            var currentPage = (int)(CoinGlassCollectorJsonHelper.TryReadLong(dataRoot, "current_page", "currentPage") ?? 1L);

            if (!dataRoot.TryGetProperty("list", out var listRoot) ||
                listRoot.ValueKind != JsonValueKind.Array)
            {
                throw new InvalidOperationException("CoinGlass Hyperliquid 持仓排行接口返回结构异常，未找到 list 数组");
            }

            var items = new List<HyperliquidPositionItem>();
            foreach (var item in listRoot.EnumerateArray())
            {
            ...
            return new ParsedHyperliquidPositionResponse(symbol, totalPages, currentPage, items);
        }

        private static HyperliquidPositionItem? TryParseItem(JsonElement item)
        {
            ...
            return new HyperliquidPositionItem(
                User: user.Trim(),
        ...
        private sealed record ParsedHyperliquidPositionResponse(
            string Symbol,
            int TotalPages,
            int CurrentPage,
            IReadOnlyList<HyperliquidPositionItem> Items);

        private readonly record struct HyperliquidPositionItem(
            string User,
            string Symbol,
