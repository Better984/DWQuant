...
{
    /// <summary>
    /// CoinGlass Hyperliquid 钱包持仓分布采集器。
    /// </summary>
    public sealed class CoinGlassHyperliquidWalletPositionDistributionCollector : IIndicatorCollector
    {
        private readonly CoinGlassClient _coinGlassClient;
        private readonly CoinGlassOptions _coinGlassOptions;
        private readonly ILogger<CoinGlassHyperliquidWalletPositionDistributionCollector> _logger;

        public CoinGlassHyperliquidWalletPositionDistributionCollector(
            CoinGlassClient coinGlassClient,
            IOptions<CoinGlassOptions> coinGlassOptions,
            ILogger<CoinGlassHyperliquidWalletPositionDistributionCollector> logger)
        {
        ...
        public string CollectorName => "coinglass.hyperliquid_wallet_position_distribution.collector";
        ...
        public bool CanHandle(IndicatorDefinition definition)
        {
            ...
            return string.Equals(definition.Provider, "coinglass", StringComparison.OrdinalIgnoreCase)
                && string.Equals(definition.Code, "coinglass.hyperliquid_wallet_position_distribution", StringComparison.OrdinalIgnoreCase);
        }
        ...
        public async Task<IndicatorCollectResult> CollectAsync(IndicatorDefinition definition, string scopeKey, CancellationToken ct)
        {
            _logger.LogInformation(
                "[coinglass][Hyperliquid][钱包持仓分布] 开始拉取钱包持仓分布: scopeKey={ScopeKey}",
                scopeKey);

            using var response = await _coinGlassClient
                .GetHyperliquidWalletPositionDistributionAsync(ct)
            ...
            {
                throw new InvalidOperationException("CoinGlass Hyperliquid 钱包持仓分布接口返回为空，无法生成指标数据");
            }

            var displayItems = orderedItems
                .Take(Math.Max(1, _coinGlassOptions.HyperliquidWalletPositionDistributionTopCount))
                .ToList();
        ...
        private static IReadOnlyList<HyperliquidWalletDistributionItem> ParseResponse(JsonElement root)
        {
            CoinGlassCollectorJsonHelper.ValidateResponseCode(root, "Hyperliquid 钱包持仓分布");

            if (root.ValueKind != JsonValueKind.Object ||
                !root.TryGetProperty("data", out var dataRoot) ||
                dataRoot.ValueKind != JsonValueKind.Array)
            {
                throw new InvalidOperationException("CoinGlass Hyperliquid 钱包持仓分布接口返回结构异常，未找到 data 数组");
            }

            var items = new List<HyperliquidWalletDistributionItem>();
        ...
        private static HyperliquidWalletDistributionItem? TryParseItem(JsonElement item)
        {
            ...
            return new HyperliquidWalletDistributionItem(
        ...
        private readonly record struct HyperliquidWalletDistributionItem(
            string GroupName,
