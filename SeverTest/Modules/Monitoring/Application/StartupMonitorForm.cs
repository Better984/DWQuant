...
namespace ServerTest.Modules.Monitoring.Application
{
    public sealed class StartupMonitorForm : Form
    {
        ...
        private readonly int _maxLogItems;
        ...
        private readonly int _maxTradingLogItems;
        ...
        private List<MarketDataGap> _detectedGaps = new();
            ...
            IServiceProvider serviceProvider)
        {
            ...
            var options = monitoringOptions ?? new MonitoringOptions();
            _maxLogItems = options.MaxLogItems;
            _maxTradingLogItems = options.MaxTradingLogItems;
        ...
        public void AppendLog(StartupMonitorLogEntry entry)
        {
            ...
            if (_logEntries.Count > _maxLogItems)
            {
            ...
            if (isStateLog || isRunLog)
            {
                ...
                if (_tradingLogEntries.Count > _maxTradingLogItems)
                {
...
private Panel BuildNetworkPanel()
        ...
        private void AppendTradingLog(StartupMonitorLogEntry entry, bool isStateLog)
        {
            ...
            if (_tradingLogList.Items.Count > _maxTradingLogItems)
            {
        ...
        private async Task SortDataByTimeAsync()
        {
            ...
            try
            {
                ...
                var repository = _serviceProvider.GetRequiredService<MarketDataMaintenanceRepository>();
                ...
                // 检查表是否存在
                var exists = await repository.TableExistsAsync(tableName, default);
                if (!exists)
                {
                    AppendDownloadLog(new DownloadLogEntry
                    {
                ...
                var rowsAffected = await repository.SortByOpenTimeAsync(tableName, default);
        ...
        private async Task CheckBaseDataIntegrityAsync()
        {
            ...
            try
            {
                ...
                // 检查数据完整性
                var gapList = await repository.QueryGapsAsync(tableName, timeframeMs, 20, default);
                ...
                else
                {
                    ...
                    foreach (var gap in gapList)
                    {
                        ...
                        {
                            Level = DownloadLogLevel.Warning,
                            Message = $"缺失: {gap.GapStartTime} -> {gap.GapEndTime} (约{gap.GapMinutes:F1} 分钟)"
                        });
                    }
                ...
                // 检查重复
                var duplicateCount = await repository.GetDuplicateCountAsync(tableName, default);
                if (duplicateCount > 0)
                {
                    AppendDownloadLog(new DownloadLogEntry
                    {
                    ...
                    try
                    {
                        ...
                        if (gapList.Count > 0)
                        {
                            ...
                            foreach (var gap in gapList.Take(5)) // 只显示前 5 个
                            {
                                ...
                                {
                                    Level = DownloadLogLevel.Warning,
                                    Message = $"[{tf}] 缺失: {gap.GapStartTime} -> {gap.GapEndTime} (约{gap.GapMinutes:F1} 分钟)"
                                });
                            }
                            if (gapList.Count > 5)
                            {
        ...
        private async Task AggregateTimeframesAsync()
        {
            ...
            try
            {
                ...
                var exists = await repository.TableExistsAsync(tableName1m, default);
                if (!exists)
                {
                    AppendDownloadLog(new DownloadLogEntry
                    {
                ...
                var totalCount = await repository.GetRowCountAsync(tableName1m, default);
                AppendDownloadLog(new DownloadLogEntry
                {
                    Level = DownloadLogLevel.Info,
                ...
                var timeRange = await repository.GetOpenTimeRangeAsync(tableName1m, default);
                ...
                foreach (var tf in allTimeframes)
                {
                    try
                    {
                        ...
                        await repository.CreateTableLikeAsync(targetTableName, tableName1m, default);

                        var existingCount = await repository.GetRowCountAsync(targetTableName, default);
                        AppendDownloadLog(new DownloadLogEntry
                        {
                            Level = DownloadLogLevel.Info,
                        ...
                        for (var start = rangeStart; start < rangeEndExclusive; start += batchMs)
                        {
                            ...
                            var batchInserted = await repository.InsertAggregateBatchAsync(
                                tableName1m,
                                targetTableName,
                                tfMs,
                                start,
        ...
        private async Task TryFillGapsAsync()
        {
            ...
            try
            {
                ...
                foreach (var gap in _detectedGaps)
                {
                    try
                    {
                        var gapStart = gap.GapStartTime;
                        var gapEnd = gap.GapEndTime;
                        // 计算下载范围：前一天到后一天
                        ...
                        var filledCount = await repository.CountRowsInRangeAsync(tableName, gapStartMs, gapEndMs, default);
