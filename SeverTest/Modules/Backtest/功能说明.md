...
- 支持超级管理员压测入口：可通过 `AdminUserController` 批量创建测试策略并并发提交回测任务，回传提交性能与线程快照。
...
- 批量模式新增开仓执行过滤：按 `MaxPositionQty` 在“候选开仓 → 平仓模拟”之间执行同向持仓上限阻断，超限候选不纳入交易统计。
- 取消任务口径收敛：仅允许取消 `queued` 状态任务，避免运行中中断导致结果不一致。
- 已支持多策略组合回测（P1）：请求可携带 `strategies` 与 `strategyScopeKey`，服务端按策略腿顺序执行并合并为一份组合资金曲线与统计结果。
- 组合资金模型（P1）：按“共享单份初始资金 + 各策略净PnL叠加”构建总权益曲线，输出 `totalEquityCurveRaw` 与 `totalEquitySummary`。
- 组合结果追踪：`BacktestSymbolResult` 与 `BacktestTrade` 增加 `strategyScopeKey` / `strategyIndex`，便于定位交易来源策略腿。
- 多策略增强（P2 可选）：可升级为“统一时间轴组合执行 + 执行层资金约束”，用于进一步提升语义一致性与峰值性能。
- 支持分布式回测：核心节点可将队列任务分发到通过 WebSocket 接入的远端算力节点执行。
- 支持算力出租模式：高性能本地机器或志愿者节点运行 Worker 后即可接入核心节点领取任务。
- 算力节点执行任务增加超时保护（读取 `Backtest:TaskTimeoutMinutes`），避免异常任务长期占用分布式执行槽位。
...
| 文件 | 职责 |
|---|---|
| `BacktestService.cs` | 回测入口服务，负责单策略执行与多策略组合编排、结果合并、输出裁剪 |
...
| `BacktestWorkerDispatchHostedService.cs` | 核心节点分发器（将队列任务下发到在线算力节点） |
| `BacktestWorkerClientHostedService.cs` | 算力节点客户端（连接核心节点、接收并执行任务） |
| `BacktestWorkerGateway.cs` | 核心节点算力接入网关（`/ws/worker`） |
| `BacktestWorkerRegistry.cs` | 核心节点在线算力节点注册中心 |
| `BacktestWorkerMessageService.cs` | 核心节点处理算力回传（进度/结果/断线回收） |

### 依赖注入约束
- `BacktestWorkerMessageService` 依赖 `BacktestTaskRepository`（作用域仓储），二者需保持作用域生命周期（`AddScoped`）。
- 禁止将 `BacktestWorkerMessageService` 注册为单例，否则会触发 `Cannot consume scoped service ... from singleton ...`。
- `BacktestWorkerGateway` 保持作用域生命周期，通过当前请求作用域解析 `BacktestWorkerMessageService` 与仓储依赖链。
- `BacktestWorkerDispatchHostedService` 为 HostedService（单例），禁止构造函数直接注入 `BacktestTaskRepository`；需在执行循环中通过 `IServiceProvider.CreateScope()` 获取仓储。
...
| `BacktestWorkerModels.cs` | 算力节点注册、心跳、执行、进度、结果等模型 |
...
- 高速模式开仓过滤：在统一平仓前按 `MaxPositionQty` 执行同向持仓累计校验，`当前在途持仓 + 本次候选开仓 > 上限` 时阻断该候选。
...
3. 任务执行路径按服务器角色分流：
   - **`full` 角色**：`BacktestTaskWorker` 在本机轮询并执行回测任务；
   - **`core` 角色**：`BacktestWorkerDispatchHostedService` 将队列任务分发给在线算力节点；
   - **`backtest-worker` 角色**：`BacktestWorkerClientHostedService` 连接核心节点，接收并执行任务。
4. 前端通过以下方式获取进度/结果：
...
6. 任务取消仅支持 `queued`：`running` 任务不再允许取消。
...
- **终态防覆盖**：任务完成/失败与进度更新仅在 `running` 状态下写入，避免取消或回队列后被旧回传覆盖。
...
- 回测任务存储在 MySQL `backtest_task` 表，包含：
  - 任务元信息（userId、reqId、assignedWorkerId、exchange、symbols、timeframe、barCount）
...
- `EnsureSchemaAsync` 兼容旧版本 MySQL：通过 `information_schema` 检查字段/索引存在性后再执行 `ALTER TABLE`，避免 `ADD COLUMN IF NOT EXISTS` 语法不兼容导致启动失败。
- `assigned_worker_id` 字段用于记录当前处理任务的算力节点，节点断线时核心节点会将任务回队列重试。
...
- 分布式算力握手鉴权采用请求头 `X-Worker-Key`，避免将密钥拼接在连接 URL 中。
...
分布式回测相关配置（`BacktestWorker`）：

| 配置键 | 类型 | 说明 | 实时生效 |
|---|---|---|---|
| `BacktestWorker:DispatchPollingIntervalMs` | int | 核心节点任务分发轮询间隔（毫秒） | 否（需重启） |
| `BacktestWorker:HeartbeatSeconds` | int | 算力节点心跳间隔（秒） | 否（需重启） |
| `BacktestWorker:ReconnectDelaySeconds` | int | 算力节点断线重连间隔（秒） | 否（需重启） |
| `BacktestWorker:MaxParallelTasksPerWorker` | int | 单算力节点并行任务上限 | 否（需重启） |
| `BacktestWorker:CoreWsUrls` | string | 核心节点地址列表（`|` 分隔，按顺序重连） | 否（需重启） |

算力节点接入必填项（`appsettings.json`）：
- `BacktestWorker:CoreWsUrls`：核心节点地址列表（示例：`127.0.0.1:9635|192.168.1.10:9635`，支持自动补全为 `ws://.../ws/worker`）。
- `BacktestWorker:CoreWsUrl`：单地址兼容字段；当 `CoreWsUrls` 为空时回退使用该字段。
- `BacktestWorker:AccessKey`：算力节点鉴权密钥（核心节点与算力节点保持一致）。
- `BacktestWorker:WorkerId`：算力节点唯一标识（为空时自动使用 `MachineName-PID`）。
- 连接时密钥通过 `X-Worker-Key` 请求头传递，URL 仅携带 `workerId`。

启动交互流程（`ServerRoleBootstrap`）：
1. 选择服务器模式（`core` / `backtest-worker` / `full` / `live-trading-worker`）。
2. 选择是否新增/更新分布式核心节点地址（`|` 分隔，多地址持久化到 `Config/server-role.local.json`）。
3. 确认启动，确认后按最新选择写入本地持久化文件。
...
- 前端回测联动（`StrategyDetailDialog` + `MarketChart`）在开发环境新增调试日志，覆盖仓位点击、focusRange 变更、`marketdata.kline.history` 请求与图表绘制阶段，便于端到端排查“点击仓位后为何展示某段 K 线”问题。
- 仓位聚焦绘图按同一时间区间拆分为互不重叠的价格区域：预设止盈（浅绿）、预设止损（红）与已参与成交区（盈利深绿 / 亏损深红）；区域采用纯填充绘制（无边框），并额外绘制开仓价时间点到平仓价时间点的虚线连线。
- 仓位聚焦会根据图表当前最大可见 K 线数量自动评估是否需要放大周期；若当前周期无法完整容纳该持仓区间，会自动切换到可一次性展示全区间的更大周期（如 1m 自动切到 5m/15m）。
- 仓位聚焦状态支持左右滑动按需加载更多行情：左滑补充更早历史 K 线，右滑补充更新区间 K 线，便于继续查看持仓前后上下文。
- 右侧滑动加载增加防抖与去重：仅回填“比当前最右时间更新”的 K 线，且禁止自动链式追拉，避免轻微右移触发连续请求与视图跳转循环。
- 右侧滑动加载增加“边界阈值 + 在途保护 + 视图稳态补偿”：仅在接近最右边界时触发一次，补数据后按锚点像素回滚位移，降低图表跳到最新处的突兀感。
- 仓位聚焦绘图会在首次滚动后追加一次“像素级居中校准”，按区间起止时间点的横坐标计算偏移量，自动将仓位区间尽量对齐到图表中部。
- 回测工作台 K 线绘图工具支持“十字星”：用户选择十字星后单击一个点，会生成穿过该点的横向与纵向无限延展线，并在交点显示中心标记；支持后续选中与右键删除，缩放、平移、切换周期时保持锚定同步。
...
- 批量模式增强阶段推送：
  ...
  - 第二阶段会附带 `blockedPositions`，显示 `MaxPositionQty` 阻断数量；
...
## 前端回测卡片（`backtest-card`）性能报告
- 策略详情回测卡片在回测完成后，会基于 `backtest.run` 结果与 `backtest.progress` 阶段快照生成“性能报告”。
- 报告默认展示：总耗时、Bar 吞吐、交易吞吐、平均每 Bar 耗时、平均每笔交易耗时、结果输出体积（交易/曲线/事件）。
- “阶段耗时明细”会按阶段展示耗时、占比、进度、关键计数（处理量/仓位/阻断/胜负）与完成状态，便于定位慢点。
- “瓶颈分析”会基于阶段耗时占比、数据加载占比、结果汇总耗时、`MaxPositionQty` 阻断比例与整体吞吐自动给出高/中/提示级建议。
- 回测运行中若处于 `batch_close_phase`，会实时显示阻断仓位计数（`blockedPositions`），方便观察仓位上限对结果的影响。
...
- `Application/BacktestWorkerDispatchHostedService.cs`（核心节点任务分发）
- `Application/BacktestWorkerClientHostedService.cs`（算力节点执行客户端）
- `Application/BacktestWorkerGateway.cs`（核心节点算力接入网关）
- `Application/BacktestWorkerRegistry.cs`（在线算力节点注册中心）
- `Application/BacktestWorkerMessageService.cs`（算力回传处理）
- `Controllers/BacktestController.cs`
- `Domain/BacktestModels.cs`
- `Domain/BacktestWorkerModels.cs`
...
- `Shared/Application/Services/ServerRoleBootstrap`：提供核心节点/算力节点/全功能节点角色切换。
- `Accounts`：`AuthTokenService` 用于 API 鉴权获取 userId。
- `AdminBroadcast`：超级测试台通过 `admin.user.test.backtest-stress` 调用回测任务提交链路，执行回测压测与状态抽样。
