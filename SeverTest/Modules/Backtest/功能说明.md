...
- 回测开仓前复用与实盘一致的 `StrategyTradeTarget + 平台风控链`（基础参数 / 市价数量归一化 / 最大持仓），保证数量精度与阻断口径一致。
- 回测与策略执行共用指标 warmup 口径：通过 `TalibCalcRules.ResolveRequiredBars` 统一计算 `requiredBars`，EMA 族指标（`EMA/DEMA/TEMA/TRIX/T3/KAMA/MAMA`）最小预热 200 根（含 offset 偏移安全余量）。
- 支持多标的交集时间轴、固定滑点、固定下单数量、止盈止损、自动反向。
- 支持异步任务队列：用户提交回测后获取 `taskId`，通过 HTTP 轮询或 WebSocket 推送跟踪进度与结果。
- 输出交易明细、权益曲线、事件与统计结果（可裁剪），含高级统计指标。
- 回测结束不再对未触发止盈止损的仓位做“收尾强平”；改为输出未平仓快照（`BacktestTrade.isOpen=true`），用于前端未平仓样式展示与定位。
...
| `BacktestMainLoop.cs` | 主循环执行器（指标计算、条件评估、动作执行、风控、收尾持仓标记） |
...
- 目标与风控：`StrategyTradeTargetModels` + `StrategyTargetRiskService`。
...
- 收尾口径：回测结束时，仍在持仓的记录以未平仓快照输出（`isOpen=true`），不计入已完成平仓统计。
...
- 胜率 `WinRate`：盈利平仓交易数 / 已完成平仓交易数（未平仓快照不参与统计）。
- 交易次数 `TradeCount`。
...
1. 选择服务器模式（`core` / `backtest-worker` / `full`）。
...
- 本地回测工作台新增“专业”摘要页：基于前端本地回测结果直接计算并展示已平仓成本归因、Long/Short 分解、平仓原因分解、Top 回撤区间、月度收益热力图与近 N 笔滚动质量（滚动净盈亏 / 胜率），无需依赖后端新增统计字段。
- 专业页中的回撤区间基于当前权益预览序列计算；当权益点位超过前端预览上限时，页面会提示“预览点 / 总点数”，用于提醒超长样本下该视图更适合做结构观察而非逐点复盘。
## 回测实时推送
...
- `Application/BacktestActionExecutor.cs`（复用目标层与平台风控链）
...
- `TradingExecution`：复用目标模型与平台风控链，保证市价单数量校验一致。
...
## 变更记录
- 2026-03-06：`BacktestActionExecutor` 接入与实盘一致的 `StrategyTradeTarget` 和 `StrategyTargetRiskService`，回测开仓前统一执行基础参数校验、市价数量归一化和最大持仓限制。
