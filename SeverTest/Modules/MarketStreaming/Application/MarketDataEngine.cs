...
namespace ServerTest.Modules.MarketStreaming.Application
{
    ...
    public class MarketDataEngine : BaseService, IMarketDataProvider
    {
                            ...
                            if (previousCandle.HasValue)
                            {
                                ...
                                if (closedTimestamp > 0)
                                {
                                    ...
                                    Logger.LogInformation(
                                        "[行情收线检测] 交易所={Exchange} 交易对={Symbol} 周期={Timeframe} K线时间={CandleTime} 本地时间={LocalTime}",
                                        exchangeId,
                                        symbol,
                                        "1m",
                                        DateTimeOffset.FromUnixTimeMilliseconds(closedTimestamp).ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss.fff"),
                                        nowLocal.ToString("yyyy-MM-dd HH:mm:ss.fff"));
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (finalizedTimestamp > 0)
                            {
                                var nowLocal = DateTime.Now;
                                Logger.LogInformation(
                                    "[聚合周期收线] 交易所={Exchange} 交易对={Symbol} 周期={Timeframe} K线时间={CandleTime} 本地时间={LocalTime}",
                                    exchangeId,
                                    symbol,
                                    timeframe,
                                    DateTimeOffset.FromUnixTimeMilliseconds(finalizedTimestamp).ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss.fff"),
                                    nowLocal.ToString("yyyy-MM-dd HH:mm:ss.fff"));
            ...
            bool isBarClose = true)
        {
            ...
            var nowLocal = DateTime.Now;
            Logger.LogInformation(
                "[行情任务发布] 交易所={Exchange} 交易对={Symbol} 周期={Timeframe} 收线={IsBarClose} K线时间={CandleTime} 本地时间={LocalTime}",
                exchangeId,
                symbol,
                timeframe,
                isBarClose,
                DateTimeOffset.FromUnixTimeMilliseconds(candleTimestamp).ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss.fff"),
                nowLocal.ToString("yyyy-MM-dd HH:mm:ss.fff"));
