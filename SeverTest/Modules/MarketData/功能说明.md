...
- 支持历史K线离线包打包（按周期可配置保留天数）并定时上传至阿里云 OSS。
- 支持云端完整性巡检（latest-manifest、版本清单、分片对象存在性检测）与缺失补给。
- 支持离线包旧版本自动清理（按保留版本数+保留天数双策略控制）。

## 配置选项
- `HistoricalMarketDataOptions`：历史行情同步、预加载与缓存上限。
- `MarketDataQueryOptions`：历史数据拉取批次与单次请求上限。
- `HistoricalDataPackageOptions`：离线包启用开关、更新间隔、交易所/币对范围、周期保留天数、OSS 目录前缀、云端巡检开关、缺失自动补给、版本清理参数（保留版本数/保留天数/扫描上限/单次删除上限）。
...
- 服务：
  ...
  - `Application/HistoricalDataPackageService.cs`：历史K线离线包生成与上传服务
  - `Application/HistoricalDataPackageHostedService.cs`：历史K线离线包定时任务
...
- 前端可通过 `marketdata.offline-package.manifest` 查询最新离线包清单（manifest + 数据分片 URL），下载后落本地 IndexedDB 供图表与本地回测使用。
- 后端在每轮离线包任务开始前执行云端巡检：识别“首次上传”“latest-manifest 丢失”“分片缺失”等状态，并在开启自动补给时执行恢复动作（恢复 latest-manifest 别名或重新生成上传）。
- 后端在每轮上传后执行版本清理：默认保留最近版本与最近天数范围内的数据，超出范围的旧版本按限额删除，避免 OSS 存储持续膨胀。
