...
namespace ServerTest.Modules.MarketData.Application
{
    ...
    public sealed class BinanceHistoricalDataDownloader : BaseService
    {
        ...
        public BinanceHistoricalDataDownloader(
        ...
        private async Task EnsureTableAsync(string tableName, CancellationToken ct)
        {
            ...
            await _repository.EnsureKlineTableAsync(tableName, ct).ConfigureAwait(false);
        }
            ...
            CancellationToken ct)
        {
            ...
            var totalInserted = 0;
            var batch = new List<HistoricalMarketDataFullInsertRow>(_queryOptions.DefaultBatchSize);
            ...
            try
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    {
                        batch.Add(row);
                        if (batch.Count >= _queryOptions.DefaultBatchSize)
                        {
        ...
        private Task<int> InsertBatchAsync(
            string tableName,
            List<HistoricalMarketDataFullInsertRow> rows,
            IUnitOfWork uow,
            CancellationToken ct)
        {
            return _repository.InsertFullRowsAsync(tableName, rows, uow, ct);
        }

        private static bool TryParseLine(string line, out HistoricalMarketDataFullInsertRow row)
        {
            row = new HistoricalMarketDataFullInsertRow();
