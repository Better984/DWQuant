...
## 条件检测支持
- 比较方法：`GreaterThan`、`GreaterThanOrEqual`、`LessThan`、`LessThanOrEqual`、`Equal`、`CrossUp`、`CrossDown`、`CrossAny`。
- 兼容说明：`CrossOver` 作为 `CrossUp` 的历史别名仍可识别。
- 参数来源：优先使用 `args` 中的左右值；当 `args` 不足时可使用 `param` 作为常量参与比较。
