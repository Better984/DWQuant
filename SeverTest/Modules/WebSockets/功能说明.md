...
- `KickOld` 在多次抢占仍失败时，新增“兜底清理”：直接清理 `userId+system` 的 Redis 占位后再尝试一次，避免孤儿占位长期阻断握手。
- 同一连接的文本消息发送采用串行化控制，避免行情广播与请求响应并发发送导致的随机异常。
- 握手阻断诊断日志统一使用前缀 `[websocket 问题排查]`，覆盖 `/api/ws/handshake/check` 与 `/ws` 关键拦截分支，便于定位连接被哪一步阻断。
...
- 2026-03-02：新增 WebSocket 握手问题排查日志，统一前缀 `[websocket 问题排查]`，输出 `traceId/userId/system/ip` 等上下文。
- 2026-03-02：`KickOld` 增加“占位兜底清理后重试”，在重试抢占失败时直接清理 `userId+system` 占位并再次抢占，降低 Redis 孤儿占位导致的 403 连接超限。
