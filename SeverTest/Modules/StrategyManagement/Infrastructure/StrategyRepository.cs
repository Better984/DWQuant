...
{
    public sealed class StrategyRepository : BaseService
    {
        ...
        public async Task<IActionResult> Create(long uid, StrategyCreateRequest request)
        {
...
", connection, transaction);
                defCmd.Parameters.AddWithValue("@creator_uid", uid);
...
", connection, transaction);
                ...
                versionCmd.Parameters.AddWithValue("@created_by", uid);
...
", connection, transaction);
            ...
            {
                Logger.LogError(ex, "创建策略失败: uid={Uid}", uid);
                await SafeRollbackAsync(transaction);
                return StatusCode(500, ApiResponse<object>.Error("创建策略失败，请稍后重试"));
            }
        }

        public async Task<IActionResult> List(long uid)
        {
...
", connection);
        ...
        public async Task<IActionResult> OfficialVersions(long uid, long defId)
        {
...
", connection);
        ...
        public async Task<IActionResult> ListTemplate(long uid)
        {
...
", connection);
        ...
        public async Task<IActionResult> ListMarket(long uid)
        {
            try
            {
                using var connection = await _db.GetConnectionAsync();
...
", connection);
        ...
        public async Task<IActionResult> PublishMarket(long uid, StrategyMarketPublishRequest request)
        {
...
", connection, transaction);
                ...
                queryCmd.Parameters.AddWithValue("@uid", uid);
...
", connection, transaction);
                ...
                upsertCmd.Parameters.AddWithValue("@uid", uid);
...
", connection, transaction);
                ...
                updateVisibilityCmd.Parameters.AddWithValue("@uid", uid);
            ...
            {
                Logger.LogError(ex, "发布到策略广场失败: uid={Uid} usId={UsId}", uid, request.UsId);
                await SafeRollbackAsync(transaction);
...
", connection, transaction);
                ...
                ownerCmd.Parameters.AddWithValue("@uid", uid);
...
";
                ...
                updateUsCmd.Parameters.AddWithValue("@uid", uid);
...
", connection, transaction);
                ...
                await using (var reader = await defOwnerCmd.ExecuteReaderAsync())
                {
                    if (await reader.ReadAsync())
                    {
                        ...
                        shouldUpdateDef = creatorUid == uid
...
", connection, transaction);
            ...
            {
                Logger.LogError(ex, "修改策略失败: uid={Uid} usId={UsId}", uid, request.UsId);
                await SafeRollbackAsync(transaction);
                return StatusCode(500, ApiResponse<object>.Error("修改失败，请稍后重试"));
            }
        }

        public async Task<IActionResult> Publish(long uid, StrategyPublishRequest request)
        {
...
", connection, transaction);
            ...
            {
                Logger.LogError(ex, "发布策略失败: uid={Uid} usId={UsId}", uid, request.UsId);
                await SafeRollbackAsync(transaction);
                return StatusCode(500, ApiResponse<object>.Error("发布失败，请稍后重试"));
            }
        }

        public async Task<IActionResult> PublishOfficial(long uid, StrategyCatalogPublishRequest request)
        {
            return await PublishToCatalogAsync(uid, request, "official");
        }

        public async Task<IActionResult> PublishTemplate(long uid, StrategyCatalogPublishRequest request)
        {
            return await PublishToCatalogAsync(uid, request, "template");
        }

        public async Task<IActionResult> SyncOfficial(long uid, StrategyCatalogPublishRequest request)
        {
            return await SyncCatalogAsync(uid, request, "official");
        }

        public async Task<IActionResult> SyncTemplate(long uid, StrategyCatalogPublishRequest request)
        {
            return await SyncCatalogAsync(uid, request, "template");
        }

        public async Task<IActionResult> RemoveOfficial(long uid, StrategyCatalogPublishRequest request)
        {
            return await RemoveFromCatalogAsync(uid, request, "official");
        }

        public async Task<IActionResult> RemoveTemplate(long uid, StrategyCatalogPublishRequest request)
        {
            return await RemoveFromCatalogAsync(uid, request, "template");
        }

        public async Task<IActionResult> SyncMarket(long uid, StrategyMarketPublishRequest request)
        {
...
", connection, transaction);
                ...
                marketCmd.Parameters.AddWithValue("@uid", uid);
...
", connection, transaction);
            ...
            {
                Logger.LogError(ex, "同步策略广场失败: uid={Uid} usId={UsId}", uid, request.UsId);
                await SafeRollbackAsync(transaction);
                return StatusCode(500, ApiResponse<object>.Error("同步失败，请稍后重试"));
            }
        }

        public async Task<IActionResult> CreateShareCode(long uid, StrategyShareCreateRequest request)
        {
...
", connection, transaction);
                ...
                insertShareCmd.Parameters.AddWithValue("@created_by_uid", uid);
...
", connection, transaction);
                ...
                insertEventCmd.Parameters.AddWithValue("@from_uid", uid);
                insertEventCmd.Parameters.AddWithValue("@to_uid", uid);
            ...
            {
                Logger.LogError(ex, "创建分享码失败: uid={Uid} usId={UsId}", uid, request.UsId);
                await SafeRollbackAsync(transaction);
                return StatusCode(500, ApiResponse<object>.Error("创建分享码失败，请稍后重试"));
            }
        }

        public async Task<IActionResult> ImportShareCode(long uid, StrategyImportShareCodeRequest request)
        {
...
", connection, transaction);
                insertUsCmd.Parameters.AddWithValue("@uid", uid);
...
", connection, transaction);
                insertLogCmd.Parameters.AddWithValue("@uid", uid);
...
", connection, transaction);
            ...
            {
                Logger.LogError(ex, "导入分享码失败: uid={Uid} shareCode={ShareCode}", uid, request.ShareCode);
                await SafeRollbackAsync(transaction);
                return StatusCode(500, ApiResponse<object>.Error("导入失败，请稍后重试"));
            }
        }

        public async Task<IActionResult> Versions(long uid, long usId)
        {
...
", connection);
            ...
            {
                Logger.LogError(ex, "获取策略版本失败: uid={Uid} usId={UsId}", uid, usId);
...
", connection);
        ...
        public async Task<IActionResult> UpdateInstanceState(long uid, long id, StrategyInstanceStateRequest request, CancellationToken ct)
        {
...
", connection, transaction);
                ...
                if (ensureApiKey)
                {
                    ...
                    {
                        var apiKey = await GetApiKeyByIdAsync(connection, transaction, uid, requestApiKeyId);
                        if (apiKey == null)
                        {
                    ...
                    {
                        var apiKey = await GetApiKeyByIdAsync(connection, transaction, uid, effectiveExchangeApiKeyId.Value);
                        if (apiKey != null)
                        {
                            apiKeyExchangeType = apiKey.ExchangeType;
                        }
                    ...
                    {
                        var apiKey = await GetLatestApiKeyAsync(connection, transaction, uid, normalizedStrategyExchange);
                        if (apiKey != null)
                        {
                            effectiveExchangeApiKeyId = apiKey.Id;
                            apiKeyExchangeType = apiKey.ExchangeType;
...
", connection, transaction);
                        ...
                        if (creatorUid == uid && string.Equals(defType, "custom", StringComparison.OrdinalIgnoreCase))
                        {
                            var updateDefCmd = new MySqlCommand(@"
UPDATE strategy_def
SET latest_version_id = @version_id, updated_at = CURRENT_TIMESTAMP
...
", connection, transaction);
                ...
                if (ensureApiKey)
                {
                    ...
                    await TryWriteRunCheckLogAsync(runCheckResult, uid, id, nextState, normalizedStrategyExchange, tradeConfig.Symbol ?? string.Empty, ct)
...
", connection, transaction);
                ...
                if (runtimeStrategy != null)
                {
                    ...
                    if (await _ownership.TryAcquireAsync(id, ct).ConfigureAwait(false))
                    {
                        _strategyEngine.UpsertStrategy(runtimeStrategy);
                        _ownership.TrackOwned(id);
                    }
                    else
                    {
                        _strategyEngine.RemoveStrategy(id.ToString());
                        Logger.LogInformation("策略实例由其他节点托管，当前节点跳过加载: usId={UsId}", id);
                    }
                }
                else
                {
                    _strategyEngine.RemoveStrategy(id.ToString());
                    await _ownership.ReleaseAsync(id, ct).ConfigureAwait(false);
                }
            ...
            {
                Logger.LogError(ex, "更新策略状态失败: uid={Uid} usId={UsId}", uid, id);
                await SafeRollbackAsync(transaction);
        ...
        {
            Span<byte> buffer = stackalloc byte[_businessRules.ShareCodeLength];
            RandomNumberGenerator.Fill(buffer);
            var chars = new char[_businessRules.ShareCodeLength];
            for (var i = 0; i < _businessRules.ShareCodeLength; i++)
            {
                chars[i] = _businessRules.ShareCodeAlphabet[buffer[i] % _businessRules.ShareCodeAlphabet.Length];
            }

            var splitIndex = _businessRules.ShareCodeLength / 2;
            return $"{new string(chars[..splitIndex])}-{new string(chars[splitIndex..])}";
        }

        private async Task<string> GenerateUniqueShareCodeAsync(MySqlConnection connection, MySqlTransaction transaction)
        {
...
", connection, transaction);
        ...
        private string NormalizeShareCode(string raw)
        {
            ...
            var cleaned = new string(raw.Where(char.IsLetterOrDigit).ToArray())
                .ToUpperInvariant();
            if (cleaned.Length == _businessRules.ShareCodeLength)
            {
                var splitIndex = _businessRules.ShareCodeLength / 2;
        ...
        private bool IsValidShareCode(string shareCode)
        {
            var cleaned = new string(shareCode.Where(char.IsLetterOrDigit).ToArray());
            return cleaned.Length == _businessRules.ShareCodeLength;
        }
...
", connection, transaction);
        ...
        private async Task<IActionResult> PublishToCatalogAsync(long uid, StrategyCatalogPublishRequest request, string target)
        {
...
", connection, transaction);
            ...
            {
                Logger.LogError(ex, "发布策略到{Target}失败: uid={Uid} usId={UsId}", target, uid, request.UsId);
                await SafeRollbackAsync(transaction);
                return StatusCode(500, ApiResponse<object>.Error("发布失败，请稍后重试"));
            }
        }

        private async Task<IActionResult> SyncCatalogAsync(long uid, StrategyCatalogPublishRequest request, string target)
        {
...
", connection, transaction);
                ...
                insertCmd.Parameters.AddWithValue("@created_by", uid);
...
", connection, transaction);
            ...
            {
                Logger.LogError(ex, "同步{Target}策略失败: uid={Uid} usId={UsId}", target, uid, request.UsId);
                await SafeRollbackAsync(transaction);
                return StatusCode(500, ApiResponse<object>.Error("同步失败，请稍后重试"));
            }
        }

        private async Task<IActionResult> RemoveFromCatalogAsync(long uid, StrategyCatalogPublishRequest request, string target)
        {
...
", connection);
        ...
        private static IActionResult Ok<T>(ApiResponse<T> payload)
        {
            return new OkObjectResult(payload);
        }

        private static IActionResult BadRequest<T>(ApiResponse<T> payload)
        {
            return new BadRequestObjectResult(payload);
        }

        private static IActionResult Unauthorized<T>(ApiResponse<T> payload)
        {
            return new UnauthorizedObjectResult(payload);
        }

        private static IActionResult NotFound<T>(ApiResponse<T> payload)
        {
            return new NotFoundObjectResult(payload);
        }

        private static IActionResult StatusCode<T>(int statusCode, ApiResponse<T> payload)
        {
